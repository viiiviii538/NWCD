...
class CalcRiskScoreV2Test(unittest.TestCase):
    def test_empty_inputs(self):
        score, warnings = calc_risk_score_v2([], [])
        self.assertEqual(score, 0.0)
        self.assertEqual(warnings, [])

    def test_single_dangerous_port(self):
        score, warnings = calc_risk_score_v2(["3389"], ["JP"])
        self.assertEqual(score, 4.0)
        self.assertTrue(any("RDP" in w for w in warnings))

    def test_dangerous_country(self):
        score, warnings = calc_risk_score_v2([], ["CN"])
        self.assertEqual(score, 3.0)
        self.assertTrue(any("CN" in w for w in warnings))

    def test_utm_reduction(self):

        base_score, _ = calc_risk_score_v2(["3389"], ["RU"])
        utm_score, _ = calc_risk_score_v2(["3389"], ["RU"], has_utm=True)
    ...
    def test_unknown_port(self):
        score, warnings = calc_risk_score_v2(["9999"], ["JP"])
