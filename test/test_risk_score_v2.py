...
class CalcRiskScoreV2Test(unittest.TestCase):
    ...
    def test_dangerous_country(self):
        score, warnings = calc_risk_score([], ["CN"])
        self.assertEqual(score, 3.0)
        self.assertTrue(any("CN" in w for w in warnings))

    def test_utm_reduction(self):

        base_score, _ = calc_risk_score(["3389"], ["RU"])
        utm_score, _ = calc_risk_score(["3389"], ["RU"], has_utm=True)
    ...
    def test_calc_utm_items(self):
        ports = ["3389"]
        countries = ["CN"]
        score, _ = calc_risk_score(ports, countries, True)
        utm = calc_utm_items(score, ports, countries)
        self.assertEqual(utm, ["firewall", "ips", "web_filter"])

    def test_mixed_countries(self):
        score, warnings = calc_risk_score([], ["JP", "RU"])
        self.assertEqual(score, 3.0)
    ...
    def test_unknown_port(self):
        score, warnings = calc_risk_score(["9999"], ["JP"])
