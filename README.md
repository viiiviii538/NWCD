...
`risk_score.py` スクリプトは、各機器の開放ポートと通信先の国情報を入力とし、0〜100 の危険度スコアを算出します。入力ファイルは次のような JSON 配列を想定しています。
